...
void ADC_start() {
    ...
    MODIFY_REG(ADC12_COMMON->CCR, ADC_CCR_DUAL_Msk, 0b00000u << ADC_CCR_DUAL_Pos);
...
void DMA_start() {
    ...
    // 01: 16-bits
    // 10: 32-bits
    DMA1_Channel1->CCR |= (0b00u << DMA_CCR_PSIZE_Pos);
    // Memory size
    DMA1_Channel1->CCR |= (0b00u << DMA_CCR_MSIZE_Pos);
